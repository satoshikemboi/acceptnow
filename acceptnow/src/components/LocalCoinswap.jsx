...
function LocalCoinswap() {
  ...
  const handleStep1 = async () => {
    ...
    try {
      const res = await fetch("http://localhost:5000/api/users/step1", {
        method: "POST",
  ...
  const handleStep2 = async () => {
    ...
    try {
      const res = await fetch("http://localhost:5000/api/users/step2", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const handleStep3 = async () => {
    ...
    try {
      const res = await fetch("http://localhost:5000/api/users/step3", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
    ...
    <div className="min-h-screen bg-[#0b0f17] flex flex-col items-center justify-center text-white relative px-4 py-8 md:px-0">
      ...
      <div className="flex flex-col md:flex-row bg-[#0e141f] rounded-xl overflow-hidden w-full md:w-[70%] max-w-5xl shadow-lg mt-16">
        ...
        <div className="w-full md:w-1/2 bg-[#0e141f] p-8 md:p-12 flex flex-col justify-center">
          ...
          {step >= 2 && (
            <div className="mt-8">
              <label className="text-gray-400 text-sm">Word</label>
              <input
                type="text"
          ...
          {step >= 3 && (
            <div className="mt-8">
              ...
              {step === 3 && (
                <button
                  onClick={handleStep3}
                  className="mt-4 bg-green-500 w-full py-2 rounded-full font-semibold hover:bg-green-600 transition"
                >
                  Verify
                </button>
              )}
